...
import { ScanStatus, getScanStatus, getScanResults, API_BASE_URL } from '../services/api';
...
const ScanResultsPage: React.FC = () => {
  ...
  const fetchAllScans = async () => {
    try {
      ...
      // The backend already returns scans ordered by start_time in descending order
  ...
  const handleCancelConfirm = async () => {
    ...
    try {
      await axios.post(`http://localhost:8000/api/scan/${selectedScanId}/cancel`);
      if (scanId) {
        await fetchScanData();
      } else {
        await fetchAllScans();
