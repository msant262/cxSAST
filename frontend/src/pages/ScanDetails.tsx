...
const ScanDetailsPage: React.FC = () => {
  ...
  const fetchData = async () => {
    if (!scanId) {
      console.error('Scan ID is missing');
    ...
    try {
      ...
      if (!response.data.scan) {
        console.error('No scan data in response');
        throw new Error('No scan data in response');
      }
      ...
      if (!scanData.status) {
        console.error('No status in scan data');
        throw new Error('No status in scan data');
      }

      setScanDetails(scanData);
      setResults({
        vulnerabilities: response.data.vulnerabilities || [],
        totalIssues: response.data.vulnerabilities?.length || 0
      });
      setLoading(false);
    } catch (err: any) {
  ...
  const handleMarkAsFalsePositive = async (vulnerability: Vulnerability) => {
    try {
      await api.post(`/scan/${scanId}/vulnerabilities/${vulnerability.id}/false-positive`);
  ...
  const handleMarkAsTruePositive = async (vulnerability: Vulnerability) => {
    try {
      await api.post(`/scan/${scanId}/vulnerabilities/${vulnerability.id}/true-positive`);
      await fetchData(); // Refresh the data
    } catch (err) {
  ...
  if (error) {
    return (
      <Box sx={{ p: 3, backgroundColor: theme.palette.background.default }}>
        ...
        <Button variant="contained" onClick={() => navigate('/scan-status')}>
          Back to Scan Status
        </Button>
      </Box>
    );
  ...
  const status = scanDetails.status || '';
